    ...
    where TRequest : IRequest<TRespons>, ICachebleRequest
{
    ...
    {//appsettingsten baglanıp ılgılı degerı alıyoruz 
        ...
        this.cache = cache; //Get için Binder indirildi
    }
    ...
    private async Task<TRespons?> getResponseAndAddToCache(TRequest request, RequestHandlerDelegate<TRespons> next, CancellationToken cancellationToken)
    {
        ...
        DistributedCacheEntryOptions cacheOptions = new() //zamanı ekliyoruz ---> Dağıtılmış Önbellek Giriş Seçenekleri
        {
            SlidingExpiration = slidingExpiration
        };
        byte[] serilizedata = Encoding.UTF8.GetBytes(JsonSerializer.Serialize(respons));//gelen  datayı byte cevırıyoruz 
